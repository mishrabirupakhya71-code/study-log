...
export async function getDB() {
  return openDB(DB_NAME, DB_VERSION, {
    upgrade(db) {
      // subjects: {id (auto), name, color, totalMinutes}
      if (!db.objectStoreNames.contains('subjects')) {
        db.createObjectStore('subjects', { keyPath: 'id', autoIncrement: true });
      }
      // sessions: {id (auto), subjectId, subjectName, subjectColor, startTime, endTime, duration, date}
      if (!db.objectStoreNames.contains('sessions')) {
        const store = db.createObjectStore('sessions', { keyPath: 'id', autoIncrement: true });
        store.createIndex('by-date', 'date');
        store.createIndex('by-subject', 'subjectId');
      }
      // homework: {id (auto), title, subject, dueDate, priority, status, description, source, photos[], classroomId?}
      ...
      // ddays: {id (auto), title, targetDate, createdAt}
      ...
      // sync_queue: {id (auto), type, data, status, createdAt}
...
export async function addSession(session) {
  const db = await getDB();
  // Also update subject total minutes
...
export async function addHomework(hw) {
  const db = await getDB();
  return db.add('homework', { ...hw, status: 'pending', photos: [], createdAt: new Date().toISOString() });
}
...
export async function updateDailyLog(log) {
  const db = await getDB();
...
// Get study data for calendar heatmap (last N days)
